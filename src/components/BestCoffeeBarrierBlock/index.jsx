    ...
    <div className="px-2.5 !py-5 lg:!p-5 rounded-3xl bg-bg-card !mt-5 md:mt-2.5">
      ...
      <div className="flex flex-col md:flex-row gap-2.5 md:!gap-5 !mt-5 md:mt-2.5">
          ...
          {details?.map(({ icon, title, description }, index) => (
            ...
            >
              ...
              <div className="w-full">
                <h4 className="text-text-color font-primary uppercase text-22 font-medium">
                  {title}
                </h4>
                <p className="text-sm font-primary text-text-color font-normal">
                  {description}
                </p>
              </div>
            </div>
        ...
        <div className="w-full md:w-[55%] flex flex-col gap-2.5 md:!gap-5">
          <div className="w-full rounded-2xl bg-white border border-text-color/30 !p-5">
            ...
            <div className="flex flex-wrap !gap-2.5 mt-2.5">
              {certifications?.list?.map((node, index) => (
                <p
                  key={index}
                  className="text-sm leading-8 font-normal italic font-primary text-text-color !px-5 rounded-full border border-text-color/30 py-1 "
                >
                  {node}
                </p>
              ))}
          ...
          <div className="!p-5 rounded-xl bg-blue-color/25 ">
            <h4 className="text-text-color font-primary uppercase text-22 font-medium">
              {keyFeatures?.heading}
            </h4>
